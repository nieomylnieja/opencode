...
### External permissions policy

Use `permission.ask` to integrate an external approval policy. The hook fires only when OpenCode would normally ask the user.
...
export const PermissionsPlugin: Plugin = async () => {
  return {
    "permission.ask": async (input, output) => {
      const allowed = await myExternalPolicy(input)
      if (allowed) {
        output.status = "allow"
        return
      }
      ...
      output.status = "deny"
      output.message = `Denied by policy: ${input.permission}`
      // keep "ask" to fall through to the normal prompt
    },
  }
}
```
...
- `"allow"`: skip the prompt and approve
- `"deny"`: reject immediately (`output.message` is optional feedback)
- `"ask"`: continue with the normal prompt
